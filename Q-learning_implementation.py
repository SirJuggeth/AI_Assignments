...
'''
Each state (square on the board) has its own row, and the actions (to move in a different direction) are represented as columns 
in the Q-table. The Q-values in the table tell you the expected reward for each action taken from each square.
'''
...
def main():
    try:
        register(
            id='FrozenLakeNotSlippery-v1',  # Unique environment ID
    ...
    env = gym.make('FrozenLakeNotSlippery-v1', desc=None, map_name="4x4", is_slippery=False, render_mode="human")
    env.reset()
